...
async def gen_thumb(track_id: str, title: str, artist: str, duration: int, thumbnail_url: str) -> str:
    save_dir = f"database/photos/{track_id}.png"
    if await aiopath.exists(save_dir):
        return save_dir

    title, artist = clean_text(title), clean_text(artist or "Spotify")
